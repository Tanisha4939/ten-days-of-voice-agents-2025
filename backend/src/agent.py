...
# 🛡️ "FraudVision Prok" - Fraud Detection & Resolution (sqlite backend)
...
def seed_database():
    ...
    if cur.fetchone()[0] == 0:
        sample_data = [
            (
                "LILY", "4939", "4242",
                "Flip cart shopping", "$1450.00", "5:30 AM EST", "lily49.com",
                "pending_review", "Automated flag: High value transaction."
            ),
            (
                "Ruby", "9990", "1199",
                "Lance card", "$2,100.00", "6:15 pM PST", "online_transfer",
                "pending_review", "Automated flag: Unusual location."
            ),
            (
                "James", "9880", "1299",
                "Gucci", "$4,600.00", "6:15 aM PST", "online_transfer",
                "pending_review", "Automated flag: Unusual location."
            )
        ]
        cur.executemany(
...
class FraudAgent(Agent):
    def __init__(self):
        super().__init__(
            instructions="""
            You are 'Ava', an AI Fraud Detection Specialist with 
            FraudVision Pro at HDFC bank .
