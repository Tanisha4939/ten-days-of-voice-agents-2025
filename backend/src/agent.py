...
"""
Day 7 – Food & Grocery Ordering Voice Agent (SQLite) - Multi Context
- Uses SQLite DB 'order_db.sqlite'
- Seeds Multi catalog (Italian, Maxican, Indian, etc.)
- Tools:
...
def seed_database():
    """Create tables and seed the Multi catalog if empty."""
    try:
        conn = get_conn()
        cur = conn.cursor()
        ...
        if cur.fetchone()[0] == 0:
            catalog = [
            # Indian Cuisine
            ("paneer_tikka", "Paneer Tikka", "Indian", 180.00, "Homemade", "250g", "plate", json.dumps(["indian", "veg", "starter"])),
            ("butter_chicken", "Butter Chicken", "Indian", 260.00, "Restaurant Style", "350g", "plate", json.dumps(["indian", "nonveg"])),
            ("dal_tadka", "Dal Tadka", "Indian", 160.00, "Homemade", "350g", "bowl", json.dumps(["dal", "veg"])),
            ("biryani_chicken", "Chicken Biryani", "Indian", 220.00, "Hyderabadi", "500g", "box", json.dumps(["rice", "nonveg"])),

            # Italian Cuisine
            ("pasta_alfredo", "Creamy Alfredo Pasta", "Italian", 210.00, "Italian House", "350g", "box", json.dumps(["pasta", "veg"])),
            ("pizza_margherita", "Margherita Pizza", "Italian", 320.00, "Italian House", "12inch", "box", json.dumps(["pizza", "veg"])),
            ("lasagna_cheese", "Cheese Lasagna", "Italian", 350.00, "Italian House", "400g", "box", json.dumps(["veg"])),
            
            # Chinese Cuisine
            ("noodles_veg", "Veg Hakka Noodles", "Chinese", 140.00, "Wok Express", "350g", "box", json.dumps(["noodles", "veg"])),
            ("manchurian_veg", "Veg Manchurian", "Chinese", 160.00, "Wok Express", "300g", "box", json.dumps(["veg", "starter"])),
            ("fried_rice_egg", "Egg Fried Rice", "Chinese", 150.00, "Wok Express", "350g", "box", json.dumps(["rice", "egg"])),

            # Mexican Cuisine
            ("tacos_chicken", "Chicken Tacos", "Mexican", 280.00, "MexiFoods", "3 pcs", "box", json.dumps(["nonveg", "tacos"])),
            ("nachos_cheese", "Cheese Nachos", "Mexican", 180.00, "MexiFoods", "250g", "box", json.dumps(["snack", "veg"])),
            ("burrito_veg", "Veg Burrito", "Mexican", 260.00, "MexiFoods", "350g", "box", json.dumps(["veg"]))
            ]
            ...
            """, catalog)
            conn.commit()
            logger.info(f"✅ Seeded Multi catalog into {get_db_path()}")
...
RECIPE_MAP = {
    "italian pasta": ["pasta_alfredo"],
    "alfredo pasta": ["pasta_alfredo"],
    "pizza": ["pizza_margherita"],
    "margherita": ["pizza_margherita"],
    "lasagna": ["lasagna_cheese"],

    "hakka noodles": ["noodles_veg"],
    "veg noodles": ["noodles_veg"],
    "manchurian": ["manchurian_veg"],
...
async def find_item(
    ...
    query: Annotated[str, Field(description="Name or partial name of dish (e.g., 'pizza', 'biryani')")],
) -> str:
    matches = search_catalog_by_name_db(query)
    if not matches:
        return f"No items found matching '{query}'. Try generic names like pasta,tacos,biryani"
...
async def add_recipe(
    ctx: RunContext[Userdata],
    dish_name: Annotated[str, Field(description="Name of dish, e.g. 'alfredo pasta', 'hakka noodles', 'chicken tacos', 'paneer tikka'")],
) -> str:
    key = dish_name.strip().lower()
    if key not in RECIPE_MAP:
        return f"Sorry, I don't have a recipe for '{dish_name}'. Try 'chicken', 'pasta' or 'paneer butter masala'."
    added = []
...
async def ingredients_for(
    ...
    if not item_ids:
        return f"Sorry, I couldn't determine ingredients for '{request}'. Try a simpler phrase like 'noodels' or 'tacos'."
...
class FoodAgent(Agent):
    def __init__(self):
        super().__init__(
            instructions="""
            You are 'James', a helpful assistant for ' TK Food ', an Multi grocery store.
            Currency is Indian Rupees (₹).
            
            Capabilities:
            1. Catalog: Search for Indian,Italian,Maxican items ( biriani chicken, chines, tacos).
            2. Cart: Add/Remove items, Show cart.
            3. Recipes: Add ingredients for dishes like bread, Maggi, Paneer Butter Masala.
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info("\n" + "🇮🇳" * 12)
    logger.info("🚀 STARTING  Food & Grocery Ordering Voice Agent  (Multi Context + Auto-Tracking)")
