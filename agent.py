...
async def entrypoint(ctx: JobContext):
    ...
    async def handle_user_message(text: str):
        ...
        # Drink Type
        if order["drinkType"] is None and any(x in t for x in ["latte", "mocha", "espresso", "americano", "cappuccino"]):
            order["drinkType"] = t
            return "Great choice! What size would you like? Small, medium, or large?"
        
        # Size
        if order["size"] is None and any(x in t for x in ["small", "medium", "large"]):
            order["size"] = t
            return "Got it! What kind of milk would you like? Whole, oat, almond, or soy?"
        
        # Milk
        if order["milk"] is None and any(x in t for x in ["oat", "almond", "soy", "whole"]):
            order["milk"] = t
            return "Nice! Any extras? You can say whipped cream, caramel, or extra shot."
        
        # Extras
        if any(x in t for x in ["whipped", "caramel", "shot"]):
            order["extras"].append(t)
        ...
        if all(order.values()):
            ...
            return f"Thanks {order['name']}! Your {order['size']} {order['drinkType']} with {order['milk']} milk is ready. I have saved your order!"
    ...
    async def handle_and_respond(text):
